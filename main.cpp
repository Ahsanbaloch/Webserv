...
int	main(void)
{
	...
	for (int i = 0; i < Server.num_listening_sockets; i++)
		EV_SET(&listening_event[i], Server.listening_sockets[i].getSocketFd(), EVFILT_READ, EV_ADD, 0, 0, (void *)listening_sock_ident);
	if (kevent(kq_fd, listening_event, Server.num_listening_sockets, NULL, 0, NULL) == -1)
